...
export function initCanvas(canvasEl, svgEl){
  ...
  buildDefs();
  window.addEventListener('app:theme-changed', ()=>{ buildDefs(); render(); });
  ...
  canvasEl.addEventListener('mouseleave', ()=>{ dragging = null; });
  ...
  function hitTestConnector(node, x, y){
    ...
    for(let i=0;i<sources.length;i++){
      const cy = node.y - h/2 + 24 + spacing*(i+1);
      const cx = node.x + w/2 - 6;
      if ((x-cx)*(x-cx) + (y-cy)*(y-cy) <= 25) return { id: sources[i].id, kind: 'source' };
    }
    return null;
  }
}
