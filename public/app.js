...
import { subscribe, newProject, getState, serializeProject, loadProject, markSaved, setProjectId } from './ui/store.js';
...
async function saveCurrent(){
  ...
  if (!state.id) { toast('No project id'); return; }
...
async function openProjectFlow(){
  try{
    ...
    if(!res.ok){ toast('Failed to list projects', 'error'); return; }
...
async function showOpenModal(fileList){
  ...
  refreshBtn.addEventListener('click', async ()=>{
    const res = await fetch('/api/projects');
    if(res.ok){ const data = await res.json(); list = Array.isArray(data.projects)?data.projects:[]; renderList(); }
  });
  ...
  function renderList(){
    ...
    for(const fname of list){
      ...
      Object.assign(row.style, { display: 'grid', gridTemplateColumns: '1fr auto auto auto', gap: '8px', alignItems: 'center', padding: '6px 0', borderBottom: '1px solid var(--border)', color: 'var(--color-text)' });
      const name = document.createElement('div');
      name.textContent = fname;
      const openBtn = document.createElement('button'); openBtn.textContent = 'Open'; openBtn.className = 'button';
      const renameBtn = document.createElement('button'); renameBtn.textContent = 'Rename'; renameBtn.className = 'button';
      const delBtn = document.createElement('button'); delBtn.textContent = 'Delete'; delBtn.className = 'button'; delBtn.style.background = 'var(--button-bg)'; delBtn.style.borderColor = 'var(--button-border)'; delBtn.style.color = 'var(--color-text)';
      row.appendChild(name); row.appendChild(openBtn); row.appendChild(renameBtn); row.appendChild(delBtn);

      openBtn.addEventListener('click', async ()=>{
        if (getState().dirty && !confirm('Discard unsaved changes?')) return;
        const id = fname.replace(/_prj\.json$/, '');
        const getRes = await fetch(`/api/projects/${id}_prj.json`);
        if(!getRes.ok){ toast('Failed to load project', 'error'); return; }
      ...
      renameBtn.addEventListener('click', async ()=>{
        ...
        const res = await fetch(`/api/projects/${oldId}_prj.json`, {
          method: 'PATCH', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ newId })
        });
        ...
        if(listRes.ok){ const data = await listRes.json(); list = Array.isArray(data.projects)?data.projects:[]; renderList(); }
      });
